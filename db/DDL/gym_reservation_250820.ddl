...
/
BEGIN
  EXECUTE IMMEDIATE 'DROP TRIGGER trg_resv_set_facility_money';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -4080 THEN RAISE; END IF;   -- ORA-04080: 트리거 없음 → 무시
END;
...
CREATE TABLE reservation_tbl (
    ...
    resv_content      VARCHAR2(500),                           -- 신청 시 요구 사항(단순 텍스트)
    ...
    facility_money    NUMBER         DEFAULT 0 NOT NULL        -- 예약 시점 시설 이용료 스냅샷(원)
);
...
COMMENT ON COLUMN reservation_tbl.resv_content      IS '신청 시 요구 사항(단순 텍스트)';
...
COMMENT ON COLUMN reservation_tbl.facility_money    IS '예약 시점의 시설 이용료 스냅샷(원 단위, DEFAULT 0)';
...
--------------------------------------------------------------------------------
-- 3-1) 트리거: 휴무일(closed_day_tbl)인 경우 예약 차단
--    - 엑셀 표에는 FK 연계가 없고, “휴무일 예약 불가”는 금지 규칙이므로 트리거로 강제
...
END;
/
-- ✅ 비고: 공휴일 API/DB는 holiday_tbl에 적재 후, 필요 시 차단 로직 추가 가능

--------------------------------------------------------------------------------
-- 3-2) 연계 트리거: 시설 요금 스냅샷 자동 세팅
--    - INSERT/시설 변경 시, facility_tbl.facility_money 값을 복사 저장
--------------------------------------------------------------------------------
CREATE OR REPLACE TRIGGER trg_resv_set_facility_money
BEFORE INSERT OR UPDATE OF facility_id ON reservation_tbl
FOR EACH ROW
DECLARE
  v_money facility_tbl.facility_money%TYPE;   -- 시설 테이블의 이용료 타입 그대로 사용
BEGIN
  IF :NEW.facility_id IS NOT NULL THEN
    SELECT facility_money
      INTO v_money
      FROM facility_tbl
     WHERE facility_id = :NEW.facility_id;

    IF v_money IS NULL THEN
      RAISE_APPLICATION_ERROR(-20011, '해당 시설의 이용료가 등록되어 있지 않습니다.');
    END IF;

    :NEW.facility_money := v_money;           -- 스냅샷 저장
  END IF;

EXCEPTION
  WHEN NO_DATA_FOUND THEN
    RAISE_APPLICATION_ERROR(-20012, '시설 ID가 facility_tbl에 존재하지 않습니다.');
END;
/
-- ✅ 결과: 예약 입력/시설 변경 시점의 시설 이용료가 reservation_tbl.facility_money에 고정 저장됨

--------------------------------------------------------------------------------
-- 4) 더미데이터 (휴무일과 충돌하지 않도록 날짜 선택)
--    ※ member_tbl: 'hong1','hong2','hong3' 존재
--    ※ facility_tbl: 1(축구장), 2(농구장) 존재
-- 주의사항!! 추후, 시설 요금 반영되도록 INSERT 구문을 조정 필요함
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 6-1) 💀 데이터 초기화 (안전 모드) — 예제 더미만 정리 / 구조·제약 유지
--------------------------------------------------------------------------------
DELETE FROM reservation_tbl WHERE resv_id IN (1,2,3);
COMMIT;

--------------------------------------------------------------------------------
-- 6-2) 💀 DDL까지 안전 삭제 — 테스트 종료 시 사용 (기본 주석 유지 권장)
--------------------------------------------------------------------------------
/*
BEGIN EXECUTE IMMEDIATE 'DROP TRIGGER trg_resv_block_closed_day';         EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TRIGGER trg_resv_set_facility_money';       EXCEPTION WHEN OTHERS THEN NULL; END;
/
